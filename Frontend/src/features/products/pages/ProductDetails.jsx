...
export default function ProductDetails() {
    ...
    const currentImage = images[activeThumb]?.url || "";
    const formattedPrice = `$${product.price?.amount?.toFixed(2) ?? "—"} ${product.price?.currency ?? ""}`;
                    ...
                    <div style={{ display:"flex", flexDirection:"column", gap:28 }}>
                        ...
                        <div>
                            <p style={{ fontSize:11, fontWeight:600, letterSpacing:"0.1em", textTransform:"uppercase", color: t.onSurfaceVariant, marginBottom:10 }}>
                                Color — <span style={{ color: t.onSurface, fontWeight:400, textTransform:"none", letterSpacing:0 }}>{COLORS[selectedColor].name}</span>
                            </p>
                            <div style={{ display:"flex", gap:12 }}>
                                {COLORS.map((c, i) => (
                                    <button key={c.name} title={c.name} onClick={() => setSelectedColor(i)} style={{ width:28, height:28, borderRadius:"50%", cursor:"pointer", background: c.hex, border: i===selectedColor ? `2px solid ${t.primary}` : `1px solid ${t.outlineVariant}`, outline: i===selectedColor ? `2px solid ${t.outlineVariant}` : "none", outlineOffset:2, transition:"transform .2s" }} />
                                ))}
                            </div>
                        </div>
                        ...
                        <div>
                            ...
                            <div style={{ display:"grid", gridTemplateColumns:"repeat(5,1fr)", gap:8 }}>
                                {SIZES.map(s => (
                                    <button key={s} className="arks-size" onClick={() => setSelectedSize(s)}
                                        style={{ padding:"10px 0", border:`1px solid ${selectedSize===s ? t.primary : t.outline}`, background: selectedSize===s ? t.primary : "transparent", color: selectedSize===s ? t.primaryText : t.onSurface, fontFamily:"'Hanken Grotesk',sans-serif", fontSize:13, fontWeight:500, letterSpacing:"0.05em", cursor:"pointer", transition:"all .2s" }}>
                                        {s}
                                    </button>
                                ))}
                            </div>
                        ...
                        <div style={{ display:"flex", flexDirection:"column", gap:12 }}>
                            <button className="arks-add-btn" onClick={handleAddToCart}
                                style={{ width:"100%", padding:"18px 0", background: cartFeedback ? "#2d5a27" : t.primary, color: t.primaryText, border:"none", fontFamily:"'Hanken Grotesk',sans-serif", fontSize:13, fontWeight:600, letterSpacing:"0.15em", textTransform:"uppercase", cursor:"pointer", transition:"background .3s" }}>
                                {cartFeedback ? "✓ Added to Cart" : "Add to Cart"}
                            </button>
                            <button className="arks-buy-btn"
                                style={{ width:"100%", padding:"18px 0", background:"transparent", color: t.primary, border:`1px solid ${t.primary}`, fontFamily:"'Hanken Grotesk',sans-serif", fontSize:13, fontWeight:600, letterSpacing:"0.15em", textTransform:"uppercase", cursor:"pointer", transition:"background .3s" }}>
                                Buy Now
                            </button>
                        </div>
                        ...
                        <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr 1fr", gap:16, paddingTop:20, borderTop:`1px solid ${t.outlineVariant}` }}>
                            {[{ icon:"local_shipping", label:"Free Shipping" }, { icon:"eco", label:"100% Cotton" }, { icon:"verified", label:"Authentic" }].map(b => (
